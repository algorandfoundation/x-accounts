...
function makeSuggestedParams(firstValid = 1000n, lastValid = 2000n): algosdk.SuggestedParams {
  return {
    flatFee: true,
    fee: 1000n,
    firstValid,
...
function seedPending(txnBytes: Uint8Array, lastValid?: number): string {
...
function seedPendingGroup(bytes: [Uint8Array, Uint8Array], lastValid?: number): string {
  ...
  const pending = {
    ...
    senders: [decoded.txn.sender.toString(), algosdk.decodeSignedTransaction(bytes[1]).txn.sender.toString()],
    lastValid: lv,
    addedAt: Date.now(),
  }
  storage.data.set(`pending:${id}`, JSON.stringify(pending))
